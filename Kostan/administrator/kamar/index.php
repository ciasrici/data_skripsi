		...
		<div class="row">
			<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
				<div class="page-header">
					<h2 class="pageheader-title">Kamar</h2>
				</div>
			</div>
		</div>
						...
						<?php if ($data) : ?>
							...
							<tbody class='fbody'>
								<?php foreach ($data as $td) :
									$cek_kamar = mysqli_query(connect(), "SELECT kamar_id FROM user_kamar WHERE kamar_id = '$td[id_kamar]'");
								?>
									<tr>
										<td><?= $no; ?></td>
										<td><?= $td['nama_kamar']; ?></td>
										<td><?= $td['fasilitas']; ?></td>
										<td>Rp <?= number_format($td['harga_perbulan']); ?></td>
										<td>
											<div class="badge badge-<?= mysqli_num_rows($cek_kamar) == 1 ? 'danger' : 'success'; ?>">
												<?= mysqli_num_rows($cek_kamar) == 1 ? 'Sudah ada' : 'Belum ada'; ?>
											</div>
										</td>

										<td class='text-center'>
											<form method='POST' action='edit.php' class='d-inline'>
												<input type='hidden' name='id_kamar' value='<?= $td['id_kamar']; ?>'>
											...
											<form method='POST' action='function.php' class='d-inline' onclick="return confirm('Are you sure?')">
												<input type='hidden' name='id_kamar' value='<?= $td['id_kamar']; ?>'>
												<input type='submit' name='delete' Value='Delete' class='btn badge-danger btn-sm'>
											</form>
										</td>
